...
?gsub
NJfishmeasurements.r$code2 <- gsub("s.jpg", "",NJfishmeasurements.r$Code)
NJfishmeasurements.r$code3 <- gsub("s..jpg", "",NJfishmeasurements.r$code2)
NJfishmeasurements.r$code4 <- gsub("t.jpg", "",NJfishmeasurements.r$code3)
NJfishmeasurements.r <- subset(NJfishmeasurements.r, select= -c(Code, code2, code3))
view(NJfishmeasurements.r)
NJfishmeasurements.r$Code <- NJfishmeasurements.r$code4
...
NCfishmeasurements.r$Code <- NC_fishfilenames.r #would not work if the 441 didnot match up in measurements
view(NCfishmeasurements.r)


?substr
NCfishmeasurements.r$code2 <- substr(NCfishmeasurements.r$Code,1, 15)
NCfishmeasurements.r <- subset(NCfishmeasurements.r, select= -c(Code))
view(NCfishmeasurements.r)
NCfishmeasurements.r$Code <- NCfishmeasurements.r$code2
...
```{r merge tables NC}
#merge photo id with measurements
NC_photoid_measurements <- inner_join(NCfishmeasurements.final, NC_key_photoid, by = "Code")
...
Graph
```{r}
ggplot(NC_data_final) + 
  geom_point(aes(x = Date, y = Caudheight)) + 
  geom_smooth(aes(x = Date, y = Caudheight), method= "lm") +
  theme_classic()
```
...
```{r}
ggplot(NC_data_final) + 
  geom_point(aes(x = SST, y = Caudheight)) + 
  geom_smooth(aes(x = SST, y = Caudheight), method= "lm") +
